...
class Flow(object):
    ...
    def __init__(self, flow_id, source, destination, amount, env, time, bw):
        ...
        self._last_packet = 0
        self.env.add_event(Event("Start flow", self.send_packet), self._flow_id, self._flow_start)
           ...
           # Send packets up to the window size.
            while (len(self._packets_sent) - len(self._packets_time_out) < self._cwnd):
                ...
                if (self._pack_num not in self._acks_arrived):
                    ...
                    self.env.add_event(Event("Timeout", self._timeout, pack_num = self._pack_num), self._flow_id, self._ms_before_timeout)
    ...
    def _respond_to_ack(self):
        """ Update window size.
        """
        self.env.add_event(Event("Send", self.send_packet), self._flow_id, self._resend_time)
    ...
    def _timeout(self, pack_num):
        ...
        if pack_num not in self._acks_arrived:
            self.env.add_event(Event("Resend", self.send_packet), self._flow_id, self._resend_time)
            # Go back n
